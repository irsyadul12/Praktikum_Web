...
@section('content')
    <div class="d-sm-flex align-items-center justify-content-between mb-4">
        <h1 class="h3 mb-0 text-gray-800">Edit Jadwal Pelajaran</h1>
                ...
                <div class="form-group">
                    ...
                    <select name="jam_pelajaran_id" id="jam_pelajaran_id" class="form-control" required>
                        ...
                        @foreach ($jamPelajaran as $jam)
                            <option value="{{ $jam->id }}" {{ old('jam_pelajaran_id', $jadwal->jam_pelajaran_id) == $jam->id ? 'selected' : '' }}>
                ...
                <div class="form-group">
                    ...
                    <select name="kelas_id" id="kelas_id" class="form-control" required>
                        ...
                        @foreach ($kelas as $kls)
                            <option value="{{ $kls->id }}" {{ old('kelas_id', $jadwal->kelas_id) == $kls->id ? 'selected' : '' }}>{{ $kls->nama_kelas }}</option>
                        @endforeach
                    </select>
                </div>
                ...
                <div class="form-group">
                    ...
                    <select name="mata_pelajaran_id" id="mata_pelajaran_id" class="form-control" required>
                        ...
                        @foreach ($mataPelajaran as $mapel)
                            <option value="{{ $mapel->id }}" {{ old('mata_pelajaran_id', $jadwal->mata_pelajaran_id) == $mapel->id ? 'selected' : '' }}>{{ $mapel->nama_mapel }}</option>
                        @endforeach
                    </select>
                </div>
